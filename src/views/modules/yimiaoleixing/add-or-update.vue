      ...
      <el-row>
        ...
        <el-form-item  v-if="type!='info'"  label="Vaccine Type" prop="yimiaoleixing">
          <el-input v-model="ruleForm.yimiaoleixing" 
              placeholder="疫苗类型" clearable></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yimiaoleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yimiaoleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {
